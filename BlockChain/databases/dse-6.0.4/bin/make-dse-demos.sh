...
if [ "$1" != "" ]; then
  ...
  echo Unable to locate the installed DSE demos directory. Please
  echo pass the directory name in as the first argument. If this
  echo is a package installation, make sure that 'dse-demos' is
  echo installed.
  exit 1
fi
...
if [ "$2" != "" ]; then
  ...
  echo Unable to locate the installed DSE directory. Please
  echo pass the directory name in as the second argument. You will
  echo have to specify the DSE demos folder as the first argument
  echo then the DSE folder as the second argument.
  exit 1
fi
...
# some touchup work to ensure people can use the local copies properly
