...
Prerequisites: A DSE cluster with at least one datacenter running Spark.
...
the application is ready to start receiving requests.
...
To query the data as it is coming in try some of the following queries
    ...
    SELECT * FROM method_agg where url='/about/lemon/kiwi' AND http_method='DELETE' ORDER BY time DESC  LIMIT 5; 
