...
object HttpSparkStream {
  ...
  def startHttpStream(config: Config) {
    val conf = new SparkConf()
      ...
      .set("spark.cassandra.connection.keepAliveMS", "120000")
    ...
    CassandraConnector(conf).withSessionDo { session =>
      ...
      session.execute(
        """CREATE TABLE IF NOT EXISTS method_agg(
          ...
          |time timestamp,
          |count bigint,
          |PRIMARY KEY ((url,http_method), time)
          |)
        """.stripMargin
      )
     ...
     */
    requests.map(request => (request.http_method, request.uri.toString))
      .countByValue()
      .transform((rdd, time) => rdd.map { case ((m, u), c) => ((m, u), c, time.milliseconds)})
      .map { case ((m, u), c, t) => methodAggRow(time = t, url = u, http_method = m, count = c)}
